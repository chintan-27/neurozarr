...
Load one file and add it to a store to see the shape of it:
...
That's one subject, one visit, one recording — everything below is this same operation done for many files at once. Say your recordings are scattered across folders with their own naming. Describe them in a table, one row per file, generated by a short script you adjust to your filenames:
